...
export default function CompletePage() {
  ...
  console.log("foodServicePayment:",foodServicePayment)
  ...
  useEffect(() => {
    if (foodServicePayment.length > 0) {
  ...
  const funcCreateFoodServicePayment = async (price: number, status: string, methodType: string, order_id: number) => {
    ...
    if (resFood.status === 201) {
      ...
      if (resOrder.status === 200) {
        ...
        localStorage.setItem("foodServicePayment", JSON.stringify(resFood.data));
      } else {
  ...
  return (
    <div className="payment-status-container">
      <div  id="payment-status">
        ...
        {intentId && <div id="details-table">
          <table>
            <tbody>
              ...
              {/* <tr>
                <td className="TableLabel">payment method</td>
                <td id="intent-status" className="TableContent">{paymentMethodId}</td>
              </tr>
              <tr>
                <td className="TableLabel">payment method type</td>
                <td id="intent-status" className="TableContent">{paymentMethodType}</td>
              </tr> */}
            </tbody>
          </table>
          <div>
        ...
        <p>Status: {status}</p>
        <h1>Payment foodServicePaymentID</h1>
        {/* <p>Status: {foodServicePaymentID}</p> */}
        {/* Additional content for complete page */}
      </div>
        </div>}
        ...
        <a id="retry-button" href="/login/food-service/order">Test another</a>
      </div>
    </div>
  );
}